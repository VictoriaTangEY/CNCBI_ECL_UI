...
############################################################################ Set-ups
# Base upload folder
BASE_UPLOAD_FOLDER = r'/u01/Apps/EY_working/Tool/CNCBI_ECL_UI_v2.0/File_upload/'

# Base ECL Engine folder
BASE_ECL_ENGINE = r'/u01/Apps/EY_working/ECL_Engine/'
        ...
        try:
            with future_lock:
                ...
                for future, task_id in list(future_to_task_id.items()):
                    if future.done() and task_status[task_id]['status'] == 'pending':
                        try:
...
def generate_new_config_file(reporting_date: str, run_mode: str, country: str):
    ...
    try:
        ...
        new_config_path = os.path.join(os.path.dirname(CONFIG_FILE_PATH), new_config_filename)
...
def run_ecl_engine():
    try:
        ...
        #command = f'python3 {os.path.join(BASE_ECL_ENGINE, "src", "main.py")} --configPath {new_config_path}'
        command = f'python "{os.path.join(BASE_ECL_ENGINE, "src", "main.py")}" --configPath "{new_config_path}"'
...
def get_task_status(task_id):
    try:
        if task_id not in task_status:
            logger.warning(f"Task {task_id} not found in task_status. Available tasks: {list(task_status.keys())}")
            return jsonify({'error': 'Task not found', 'available_tasks': list(task_status.keys())}), 404
        
        status_info = task_status[task_id]
        logger.info(f"Task {task_id} status requested: {status_info.get('status', 'unknown')}")
