...
def main(run_config):
    ...
    if (rc.RUN_MODE in ["3", "0-5", "1-5", "2-5", "3-5"]):
        ...
        try:
            ...
            # 2. Output handle - process each DataFrame separately
            output_handler = OutputHandler(rc)

            # Process ead_df
            if ead_df is not None and not ead_df.empty:
                try:
                    ead_df_processed = output_handler.process_general(
                        ead_df, output_type='ead_df')
                    output_path = rc.outInterimPath / "interim_output_ead_schedule.csv"
                    ead_df_processed.to_csv(output_path, index=False)
                    logger.info(
                        "Exported interim_output_ead_schedule.csv with timestamp")
                except Exception as e:
                    logger.error(
                        f"Error exporting interim_output_ead_schedule.csv: {str(e)}")
            else:
                logger.info(
                    "Skipping interim_output_ead_schedule.csv - DataFrame is None or empty")

            # Process coll_df
            if coll_df is not None and not coll_df.empty:
                try:
                    coll_df_processed = output_handler.process_general(
                        coll_df, output_type='coll_df')
                    output_path = rc.outInterimPath / "interim_output_allocated_collateral.csv"
                    coll_df_processed.to_csv(output_path, index=False)
                    logger.info(
                        "Exported interim_output_allocated_collateral.csv with timestamp")
                except Exception as e:
                    logger.error(
                        f"Error exporting interim_output_allocated_collateral.csv: {str(e)}")
            else:
                logger.info(
                    "Skipping interim_output_allocated_collateral.csv - DataFrame is None or empty")

            # Process pwa_ecl_detailed
            if pwa_ecl_detailed is not None and not pwa_ecl_detailed.empty:
                try:
                    pwa_ecl_detailed_processed = output_handler.process_general(
                        pwa_ecl_detailed, output_type='pwa_ecl_detailed')
                    output_path = rc.outInterimPath / "interim_output_ecl_detailed.csv"
                    pwa_ecl_detailed_processed.to_csv(output_path, index=False)
                    logger.info(
                        "Exported interim_output_ecl_detailed.csv with timestamp")
                except Exception as e:
                    logger.error(
                        f"Error exporting interim_output_ecl_detailed.csv: {str(e)}")
            else:
                logger.info(
                    "Skipping interim_output_ecl_detailed.csv - DataFrame is None or empty")

            # Process pwa_ecl_by_deal
            if pwa_ecl_by_deal is not None and not pwa_ecl_by_deal.empty:
                try:
                    pwa_ecl_by_deal_processed = output_handler.process_all(
                        pwa_ecl_by_deal, param, output_type='pwa_ecl_by_deal')
                    output_path = rc.outInterimPath / "interim_output_ecl_by_deal.csv"
                    pwa_ecl_by_deal_processed.to_csv(output_path, index=False)
                    logger.info(
                        "Exported interim_output_ecl_by_deal.csv with timestamp")
                except Exception as e:
                    logger.error(
                        f"Error exporting interim_output_ecl_by_deal.csv: {str(e)}")
            else:
                logger.info(
                    "Skipping interim_output_ecl_by_deal.csv - DataFrame is None or empty")
    ...
    # TODO: testing the add cols, can deleter later
    if rc.RUN_MODE in ["test"]:
        print('Starting Add Cols Testing...')

        # Load interim_output_ecl_by_deal.csv
        test_file_path = rc.outInterimPath / "interim_output_ecl_by_deal.csv"
        ...
        if not test_file_path.exists():
            ...
            try:
                # Load the CSV file
                print(f"Loading test file: {test_file_path.name}")
                df_test = pd.read_csv(test_file_path, encoding='utf-8-sig')
                print(f"Loaded {len(df_test)} rows")

                # Check if param is loaded (required for add_cols)
                if param is None:
                    print("Loading parameters...")
                    param = load_parameters(paramPath=rc.paramPath)

                # Apply add_cols using AddColsHandler
                from ecl_calc.modules.io_handler._add_cols import AddColsHandler
                add_cols_handler = AddColsHandler()

                print("Applying add_cols...")
                df_test_processed = add_cols_handler.add_cols(
                    df_test.copy(), param)

                # Output the result for checking
                output_test_path = rc.outInterimPath / \
                    "test_output_ecl_by_deal_with_add_cols.csv"
                df_test_processed.to_csv(output_test_path, index=False)
                print(f"Test result exported to: {output_test_path.name}")
                print(
                    f"Columns: {len(df_test.columns)} -> {len(df_test_processed.columns)} (added {len(df_test_processed.columns) - len(df_test.columns)})")

                print('Add Cols Testing completed')

            except Exception as e:
                print(f"Error in Add Cols Testing: {str(e)}")
                raise
    ...
    if (rc.RUN_MODE in ["4", "0-5", "1-5", "2-5", "3-5", "4-5"]):
        ...
        # Run stage 3 adjustment and capture the result
        adjusted_ecl_df = ecl_adjustment_service.post_run_stage_3_adj(
            context=rc, param=param)

        # Run output adjustment and capture the result
        final_adjusted_ecl_df = ecl_adjustment_service.post_run_output_adj(
            context=rc, param=param)

        # Save the adjusted results back to the interim output file
        if final_adjusted_ecl_df is not None and not final_adjusted_ecl_df.empty:
            try:
                # # Apply OutputHandler to process the adjusted DataFrame
                # output_handler = OutputHandler(rc)
                # df_processed = output_handler.process_all(
                #     final_adjusted_ecl_df, param)

                # Export the adjusted results
                output_path = rc.outInterimPath / "interim_output_ecl_by_deal.csv"
                final_adjusted_ecl_df.to_csv(output_path, index=False)
                logger.info(
                    "Exported adjusted ECL results to interim_output_ecl_by_deal.csv")

            except Exception as e:
                logger.error(f"Error exporting adjusted ECL results: {str(e)}")
        else:
            logger.warning("No adjusted ECL results to export")
    ...
    if rc.TO_DB == "ON":
        ...
        files_to_process = [
            ...
            str(rc.outInterimPath / "interim_output_allocated_collateral.csv"),
