...
class DataTransfer:
    ...
    def convert_to_proper_separator(self, csv_path):
        """Convert CSV file to use \x1F separator and return the path to the converted file"""
        print(f"Converting {csv_path.name} to proper separator format...")

        # Read the CSV file with default separator (comma)
        df = pd.read_csv(csv_path, encoding='utf-8-sig')

        # Create a temporary file with the proper separator
        converted_csv_path = csv_path.with_stem(f"{csv_path.stem}_converted")
        df.to_csv(converted_csv_path, index=False,
                  sep='\x1F', encoding='utf-8-sig')
        print(f"Created converted CSV at: {converted_csv_path}")
    ...
    def generate_create_table(self, csv_path, table_name):
        ...
        for col, dtype in df.dtypes.items():
            ...
            # Allow NULL for all columns by default
    ...
    def create_table(self, csv_path, table_name):
        ...
        # Convert CSV to proper separator format
        converted_csv_path = self.convert_to_proper_separator(csv_path)
        ...
        try:
            ...
            # Generate the CREATE TABLE statement using the converted CSV
            create_sql = self.generate_create_table(
                converted_csv_path, table_name)
            ...
            print(f"Create table failed for: {table_name}")
            print(f"Error details: {str(e)}")
            # Don't re-raise the exception to hide detailed error messages
            return False
        finally:
            # Clean up the converted CSV file
            if os.path.exists(converted_csv_path):
                os.remove(converted_csv_path)
                print(f"Cleaned up temporary file: {converted_csv_path}")
    ...
    def import_data(self, csv_path, table_name):
        ...
        # Convert CSV to proper separator format
        converted_csv_path = self.convert_to_proper_separator(csv_path)
        converted_csv_path_str = str(converted_csv_path)
        ...
        try:
            ...
            bcp_cmd = [
                ...
                "-P", self.db_password,
                "-C", "65001",
                "-e", f"bcp_error_{table_name}.log"
            ]

            # Run BCP with shell=True to handle the hex character properly
            # Capture and suppress all output (stdout and stderr)
            result = subprocess.run(" ".join(bcp_cmd), shell=True,
                                    capture_output=True, text=True)

            if result.returncode == 0:
                print("Import successfully")
            else:
                print(f"Import failed for table: {table_name}")
                return False
        except Exception as e:
            print(f"Import failed for table: {table_name}")
            # Don't re-raise the exception to hide detailed error messages
