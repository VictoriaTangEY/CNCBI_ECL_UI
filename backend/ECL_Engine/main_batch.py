...
from utils.batch_folder_maintain import BatchFolderMaintain
...
def main(run_config):
 ...
    # **********************************************************
    # Copy param and data to output folder (skipped for reporting-only mode)
    # **********************************************************
    if rc.RUN_MODE != "6":
 ...
            print(
                f"Copied {Path(file).name} to output data folder with timestamp")
    else:
        logger.info(
            'Skipping copy param/data to output (reporting-only RUN_MODE)')
 ...
    if (rc.RUN_MODE in ["1", "0-5", "1-5"]) and (rc.ecl_adj_mode == "ON"):
 ...
    else:
        if rc.ecl_adj_mode != "ON":
            logger.info(
                'Skipping ECL Adjustment - Deal Append (ECL_ADJ_MODE is OFF)')
        else:
            logger.info(
                'Skipping ECL Adjustment - Deal Append (not in current run mode) or ECL_ADJ_MODE is OFF')
 ...
    # **********************************************************
    if (rc.RUN_MODE in ["4", "0-5", "1-5", "2-5", "3-5", "4-5"]) and (rc.ecl_adj_mode == "ON"):
 ...
        # Save stage 3 adjusted result so post_run_output_adj reads the updated data
        if adjusted_ecl_df is not None and not adjusted_ecl_df.empty:
            adjusted_ecl_df.to_csv(
                rc.outInterimPath / "interim_output_ecl_by_deal.csv", index=False)
 ...
    else:
        logger.info(
            'Skipping ECL Adjustment - Post-Run (not in current run mode) or ECL_ADJ_MODE is OFF')
 ...
    # **********************************************************
    # Data transfer to DB server (skipped for reporting-only mode)
    # **********************************************************
    if rc.TO_DB == "ON" and rc.RUN_MODE != "6":
...
 
 ...
    elif rc.TO_DB == "ON" and rc.RUN_MODE == "6":
        logger.info(
            'Skipping Output to ECL calc server (reporting-only RUN_MODE)')
    else:
 ...
        try:
            reporting_service.run_reporting(context=rc, param=param)
...
if __name__ == "__main__":
 ...
    maintainer = BatchFolderMaintain(main_fn=main)
    maintainer.execute_batch_run(args.configPath)
