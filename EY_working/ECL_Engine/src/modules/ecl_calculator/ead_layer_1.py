...
class CashflowLayerOne:
    ...
    def batch_process(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if self.parallel_by_deals:
            # Process by number of accounts
            # Split into chunks
            chunks = [unique_acct_ids[i:i + self.chunk_size]
                      for i in range(0, len(unique_acct_ids), self.chunk_size)]

            # Process chunks sequentially
            for chunk in chunks:
                try:
                    # Create a copy of data for this chunk
                    chunk_data = valid_data_df[valid_data_df[self.data_cols['acct_id']].isin(
                        chunk)].copy()
                    chunk_sched = valid_sched_df[valid_sched_df[self.sched_data_cols['acct_id']].isin(
                        chunk)].copy()
                    ...
                    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
                        ...
                        for acct_id in chunk:
                            acct_data = chunk_data[chunk_data[self.data_cols['acct_id']] == acct_id].copy(
                            )
                            acct_sched = chunk_sched[chunk_sched[self.sched_data_cols['acct_id']] == acct_id].copy(
                            )

                            if not acct_data.empty and not acct_sched.empty:
