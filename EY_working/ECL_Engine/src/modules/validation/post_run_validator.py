...
from io_handler.parameter_loader import load_parameters
...
class EnvSetting():
    def __init__(self, context):
        ...
        self.prtflo_scope = context.prtflo_scope_fa
...
class PostRunValBasic(EnvSetting):
    def __init__(self, context):
    ...
    def load_ecl_result(self):
        ...
        df_ecl_ = (pd.read_csv(self.resultPath /
                   'ECL_calculation_result_files_deal.csv'))  # TODO: change to merged result
    ...
    def load_post_param(self):
        ...
        ## TODO: if post validator combine with validator, can use loaded parameters. it will be deperiated.
...
class PostRunVal(PostRunValBasic):
    def __init__(self, context):
        super().__init__(context)
    ...
    def ecl_override_check(self, df_ecl, param_ov):
        ...
        if for_check_df['ECL_COMPARE'].sum() + for_check_df['STAGE_COMPARE'].sum() == 0:
            ...
            res_final = for_check_df.query(
                'ECL_COMPARE == 1 or STAGE_COMPARE == 1 or LC_COMPARE == 1 or MO_CHECK == 1').copy().reset_index(drop=True)
...
class PostRunValReport(PostRunVal):
    def __init__(self, context):
        super().__init__(context)
    ...
    def post_run_validation_report_out(self):
        ...
        df_ecl, df_instr, param_ov, validparm, modelparam = self.load_basic_data_post()

        # run all validation
        try:
            stepinfo = "ECL results check"
            print(stepinfo)
            # TODO: instr table contains ead = 0, how to filter out them to check.
        ...
        if len(stage_res) > 1:
            exceptionresall = []
            for key, val in stage_res[1].items():
                for i in val:
                    exceptionresall.append(
        ...
        if len(ecl_ovr_res) > 1:
            exceptionresall = []
            for key, val in ecl_ovr_res[1].items():
                for i in val:
                    exceptionresall.append(
