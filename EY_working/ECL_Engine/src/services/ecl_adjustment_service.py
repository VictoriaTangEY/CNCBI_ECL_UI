...
def pre_run_deal_append(context, param):
    ...
    expo_df_append = expo_df_append[expo_df_param['colname'].to_list()]
...
def post_run_stage_3_adj(context, param):
    ...
    # load stage_3 ecl result
    ecl_df_stage_3 = pd.read_csv(context.outInterimPath / 'ecl_df_stage_3.csv')
    logger.info(
        f"Loaded stage 3 ECL DataFrame: {len(ecl_df_stage_3)} rows, {len(ecl_df_stage_3.columns)} columns")
    ...
    original_count = len(ecl_df_stage_3)
    ecl_df_stage_3 = ecl_df_stage_3[~ecl_df_stage_3['deal_id'].isin(
        existing_deal_ids)]
    removed_count = original_count - len(ecl_df_stage_3)
    ...
    ecl_df_stage_3 = pd.concat(
        [ecl_df_stage_3, adj_param_df], ignore_index=True)

    logger.info(
        f"Added {len(adj_param_df)} new deals from adjustment parameters")
    logger.info(f"Final stage 3 DataFrame shape: {ecl_df_stage_3.shape}")

    logger.info("="*60)
    logger.info("Stage 3 adjustment complete")
    logger.info("="*60)

    return ecl_df_stage_3
...
def post_run_output_adj(context, param):
    ...
    ecl_df = pd.read_csv(context.outInterimPath / 'ecl_df.csv')
